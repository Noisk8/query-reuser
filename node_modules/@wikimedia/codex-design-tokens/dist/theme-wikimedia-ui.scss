...
$max-width-base: none;
$max-width-breakpoint-mobile: calc( 640px - $size-absolute-1 ); // A mobile device's maximum available screen width. Many older feature phones have screens smaller than this value.
$max-width-breakpoint-tablet: calc( 1120px - $size-absolute-1 ); // A tablet device's maximum available screen width. Note, the size chosen is eager to be re-evaluated with Web team and Product Analytics.
$max-width-breakpoint-desktop: calc( 1680px - $size-absolute-1 ); // A desktop device's maximum available screen width.
...
$color-link: $color-progressive; // Default Link color and states
$color-link--hover: $color-progressive--hover;
$color-link--active: $color-progressive--active;
$color-link--focus: $color-progressive--focus;
$color-link--visited: $color-visited;
$color-link--visited--hover: $color-visited--hover;
$color-link--visited--active: $color-visited--active;
$color-link-red: $color-destructive; // Red ('new') Link color and states
$color-link-red--hover: $color-destructive--hover;
$color-link-red--active: $color-destructive--active;
$color-link-red--focus: $color-destructive--focus;
$color-link-red--visited: $color-destructive--visited;
$color-link-red--visited--hover: $color-destructive--visited--hover;
$color-link-red--visited--active: $color-destructive--visited--active;
$size-icon-x-small: calc( $font-size-medium - 4px );
$size-icon-small: $font-size-medium;
$size-icon-medium: calc( $font-size-medium + 4px );
$size-toggle-switch-grip: calc( $font-size-medium * 1.25 );
...
$spacing-horizontal-button-small-icon-only: $spacing-12 - $border-width-base; // Padding should equal 2px of spacing minus the width of the border
$spacing-horizontal-button-small: $spacing-35 - $border-width-base; // Padding should equal 6px of spacing minus the width of the border
$spacing-horizontal-button-large: $spacing-100 - $border-width-base; // Padding should equal 16px of spacing minus the width of the border
$spacing-horizontal-input-text-two-end-icons: calc( $spacing-50 * 2 + $size-icon-small ); // Rely on `calc()` to make token output usable in all formats. When there are two end icons, (i.e. a clear icon and an end icon), we need to double the horizontal padding and account for the size of the extra icon. This token can be used to calculate the horizontal position of the clear icon and the padding-end of the text input.
$spacing-start-typeahead-search-figure: $spacing-75; // The amount of space between the TypeaheadSearch figure's parent component and the TypeaheadSearch figure (input icon container, search result thumbnail, and footer icon container). We want this space to be uniform so that the figures vertically line up nicely. We use the same horizontal padding as the MenuItem.
$spacing-toggle-switch-grip-start: calc( $font-size-medium * 0.375 );
$spacing-toggle-switch-grip-end: calc( $font-size-medium * 1.25 );
$border-color-input--hover: $border-color-interactive;
$border-color-input-binary: $border-color-interactive;
$border-color-input-binary--hover: $border-color-progressive--hover;
$border-color-input-binary--active: $border-color-progressive--active;
$border-color-input-binary--focus: $border-color-progressive--focus;
$border-color-input-binary--checked: $border-color-progressive;
...
$border-transparent: $border-width-base $border-style-base $border-color-transparent;
$outline-base--focus: $border-width-base solid transparent; // Enable Windows high contrast mode on certain widgets, that have default outlines overridden.
$width-toggle-switch: calc( $font-size-medium * 3 );
$height-toggle-switch: calc( $font-size-medium * 2 );
$spacing-start-typeahead-search-icon: calc( $spacing-start-typeahead-search-figure + ( $min-size-search-figure - $min-size-icon-medium ) / 2 ); // The padding required for the icon to center align with the menu item thumbnail. We calculate the difference in size and add it to the expected spacing.
$spacing-typeahead-search-focus-addition: calc( ( $spacing-start-typeahead-search-figure + $min-size-search-figure ) - ( $min-size-icon-medium + $spacing-50 ) ); // The amount the width of the input increases when it is focused to allow for the extra spacing around the search figures. The caret position should remain static. This calculates the padding-left of the input when expanded minus the padding-left of the input when not expanded. (Note that both padding values actually include `@spacing-50` as well, but it was left out of the calculation for simplicity's sake.)
...
$box-shadow-drop-small: $box-shadow-outset-small $box-shadow-color-base;
// Warning: the following token name is deprecated (Shadow shorthands should be used in place of `drop` shadows.)
$box-shadow-drop-medium: $box-shadow-outset-medium-below $box-shadow-color-alpha-base, $box-shadow-outset-medium-around $box-shadow-color-alpha-base;